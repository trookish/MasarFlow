...
No setup is needed for the AI workspace functions (create_note, read_spec, …): `dev:full` and `start` install them as OpenCode custom tools automatically before spawning the server. The one case that needs a manual step is **running your own `opencode serve`**: run `npm run tools:install` once, then **restart** the server so it registers the tools (tools load only at server start). If a chat thread reports "The OpenCode server is missing MasarFlow's workspace functions", that's the fix.
...
| `npm run tools:install` | (Re)generate the OpenCode workspace-tool files (project + global dirs); restart `opencode serve` afterwards |
