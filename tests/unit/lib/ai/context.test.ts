...
import { formatWorkspaceContext, type RagChunk, type WorkspaceSnapshot } from "@/lib/ai/context";
...
describe("formatWorkspaceContext — RAG chunks", () => {
  it("falls back to Fuse-based pickFullBodies when no ragChunks are given", () => {
    ...
    const text = formatWorkspaceContext(snapshot({ notes }), { query: "authentication" });
  ...
  it("uses ragChunks to build the full-body note section instead of the raw clipped body", () => {
    ...
    expect(text).not.toContain("Full raw body text that would normally be clipped");
  });
  ...
  it("groups multiple chunks from the same entity together, joined with a separator", () => {
    ...
    const ragChunks: RagChunk[] = [
      { entityId: "d1", kind: "doc", title: "Deployment guide", text: "Step one.", score: 0.8 },
      { entityId: "d1", kind: "doc", title: "Deployment guide", text: "Step two.", score: 0.7 },
    ];
  ...
  it("only affects notes/docs — other sections are unchanged by ragChunks", () => {
    ...
    const withRag = formatWorkspaceContext(snapshot(), { query: "x", ragChunks: [] });
